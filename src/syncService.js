...
export async function syncCloudDatabase(phone, onUpdate) {
  ...
  try {
    ...
    if (!cloudRawVal) {
      console.log('[Sync] No database found in Vercel KV. Creating clean isolated database for new account...');
      localStorage.setItem('chand_repair_tickets', JSON.stringify([]));
      localStorage.setItem('chand_products', JSON.stringify([]));
